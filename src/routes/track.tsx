  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-16 max-w-lg space-y-8">
        <div className="card-surface rounded-lg p-8 glow-purple animate-slide-up text-center">
          ...
          <h1 className="text-2xl font-semibold text-gradient mb-2">تتبع طلبك</h1>
          ...
          <form onSubmit={(e) => { e.preventDefault(); if (code.trim()) navigate({ to: "/order/$code", params: { code: code.trim() } }); }}>
            <Input value={code} onChange={(e) => setCode(e.target.value)} placeholder="ORD-XXXXXXXX" className="h-12 num text-center text-lg mb-4" />
            <Button type="submit" className="w-full h-12 gradient-primary text-white">تتبع</Button>
          </form>
        </div>

        {myOrders.data && myOrders.data.length > 0 && (
          <div className="card-surface rounded-lg p-6 animate-slide-up">
            ...
            <div className="space-y-2">
              {myOrders.data.map((o: any) => (
                <button
                  ...
                  className="w-full text-right p-3 rounded-lg bg-muted/40 hover:bg-muted transition-colors flex items-center justify-between gap-3"
                >
                  <div className="text-xs">
                    <div className="num font-bold">{o.order_code}</div>
