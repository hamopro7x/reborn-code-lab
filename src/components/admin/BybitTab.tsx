...
import {
  ...
  getBybitConvertCoins, createBybitConvertQuote, confirmBybitConvert,
...
function BybitAccountView({ isAdmin, accountId, accountName, onBack }: { isAdmin: boolean; accountId: string; accountName: string; onBack: () => void }) {
  ...
  return (
    <div className="space-y-4 text-right" dir="rtl">
      ...
      {isAdmin && <ConvertPanel accountId={accountId} onDone={refreshAll} />}
...
/* ============================ تحويل العملة (Convert) ============================ */

function ConvertPanel({ accountId, onDone }: { accountId: string; onDone: () => void }) {
  const coinsFn = useServerFn(getBybitConvertCoins);
  const quoteFn = useServerFn(createBybitConvertQuote);
  const confirmFn = useServerFn(confirmBybitConvert);

  const [open, setOpen] = usePersistentState<boolean>(`bybit_convert_open_${accountId}`, false);
  ...
  const list = useQuery({
    queryKey: ["bybit-convert-coins", accountId],
    queryFn: () => coinsFn({ data: { accountId } }),
    enabled: open,
  });

  const coins = (((list.data as any)?.coins ?? []) as Array<{ coin: string; balance: number; toCoins: string[] }>);
  const listError = (list.data as any)?.error as string | undefined;
  const from = coins.find((c) => c.coin === fromCoin);

  const toList = useQuery({
    queryKey: ["bybit-convert-to", accountId, fromCoin],
    queryFn: () => coinsFn({ data: { accountId, coin: fromCoin } }),
    enabled: open && !!fromCoin,
  });
  const toFetched = (((toList.data as any)?.coins ?? []) as Array<{ coin: string; toCoins: string[] }>);
  const toOptions: string[] =
    (toFetched.find((c) => c.coin === fromCoin)?.toCoins ?? []).length > 0
      ? toFetched.find((c) => c.coin === fromCoin)!.toCoins
      : (from?.toCoins ?? []).length > 0
        ? from!.toCoins
        : coins.map((c) => c.coin).filter((c) => c !== fromCoin);

  useEffect(() => {
    if (!quote?.expiredTime) return;
    const t = setInterval(() => {
      const s = Math.max(0, Math.round((quote.expiredTime - Date.now()) / 1000));
      setLeft(s);
  ...
  const getQuote = useMutation({
    mutationFn: () => quoteFn({ data: { accountId, fromCoin, toCoin, amount } }),
    onSuccess: (r: any) => {
      if (!r?.ok) { setQuoteError(r?.error || "تعذّر جلب السعر"); return toast.error(r?.error || "تعذّر جلب السعر"); }
      setQuoteError("");
      setQuote(r.quote);
    },
    onError: (e: any) => { setQuoteError(e?.message || "تعذّر جلب السعر"); toast.error(e?.message || "تعذّر جلب السعر"); },
  });

  const confirm = useMutation({
    mutationFn: () => confirmFn({ data: { accountId, quoteTxId: quote?.quoteTxId } }),
    onSuccess: (r: any) => {
      if (!r?.ok) return toast.error(r?.error || "فشل التحويل");
      toast.success(`تم التحويل: ${r.fromAmount} ${r.fromCoin} → ${r.toAmount} ${r.toCoin}`);
      setQuote(null);
      setAmount("");
      list.refetch();
      onDone();
  ...
  return (
    <div className="rounded-2xl border border-border/60 bg-card/70 p-4 space-y-3" dir="rtl">
      <button
        type="button"
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl border border-border/60 bg-background/60 p-3 space-y-2">
            <div className="flex items-center justify-between text-[11px] text-muted-foreground">
              <span>من</span>
              {from && <span>متوفر: {from.balance.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {from.coin}</span>}
            </div>
            <div className="flex items-center gap-2">
              <CoinSelect
                value={fromCoin}
                onChange={(v) => { setFromCoin(v); setToCoin(""); setQuote(null); }}
                options={coins.map((c) => c.coin)}
              />
              <Input
                inputMode="decimal"
                placeholder="الكمية"
              ...
              />
              {from && (
                <Button variant="outline" size="sm" className="rounded-xl" onClick={() => { setAmount(String(from.balance)); setQuote(null); }}>
                  الكل
                </Button>
              )}
            </div>
          ...
          <div className="flex justify-center">
            <span className="size-8 rounded-full bg-blue-500/15 text-blue-400 grid place-items-center">
              <ArrowDownUp className="size-4" />
            </span>
          </div>

          <div className="rounded-xl border border-border/60 bg-background/60 p-3 space-y-2">
            <div className="text-[11px] text-muted-foreground">إلى</div>
            <CoinSelect
              value={toCoin}
              onChange={(v) => { setToCoin(v); setQuote(null); }}
              options={toOptions}
              disabled={!fromCoin}
              loading={toList.isFetching}
            />
          </div>
          ...
          <div className="space-y-1 text-xs">
            <div className="flex items-center justify-between">
              ...
              <span className="font-bold tabular-nums" dir="ltr">
                {quote && quote.rate > 0
                  ? `1 ${quote.fromCoin} \u2248 ${quote.rate.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${quote.toCoin}`
                  : "--"}
              </span>
            </div>
            <div className="flex items-center justify-between">
              ...
              <span className="font-bold tabular-nums" dir="ltr">
                {quote && quote.toAmount > 0
                  ? `${quote.toAmount.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${quote.toCoin}`
                  : "--"}
              </span>
            </div>
            {quoteError && (
              <div className="rounded-lg border border-destructive/40 bg-destructive/10 p-2 text-[11px] text-destructive-foreground">
                {quoteError}
              </div>
            )}
          </div>
          ...
          {quote ? (
            <Button className="w-full rounded-xl" disabled={confirm.isPending} onClick={() => confirm.mutate()}>
              {confirm.isPending ? <Loader2 className="size-4 animate-spin ml-1" /> : null}
              تأكيد التحويل {left > 0 ? `(${left}s)` : ""}
            </Button>
          ) : (
            <Button
              className="w-full rounded-xl"
              disabled={!fromCoin || !toCoin || !amount || getQuote.isPending}
              onClick={() => getQuote.mutate()}
            >
              {getQuote.isPending ? <Loader2 className="size-4 animate-spin ml-1" /> : null}
              احصل على عرض أسعار
            </Button>
          )}
        </div>
...
}: { value: string; onChange: (v: string) => void; options: string[]; disabled?: boolean; loading?: boolean }) {
    ...
    <Select value={value} onValueChange={onChange} disabled={disabled}>
      <SelectTrigger className="w-40 rounded-xl">
        {value ? (
          ...
          <SelectValue placeholder={loading ? "جارٍ التحميل..." : "اختر العملة"} />
        )}
      </SelectTrigger>
      <SelectContent className="max-h-72">
        <div className="sticky top-0 z-10 bg-popover p-2" dir="rtl">
          <div className="relative">
            ...
            <Input
              ...
              placeholder="بحث"
              className="h-8 rounded-lg pr-7 text-xs"
            />
          </div>
        </div>
        {filtered.length === 0 && (
