  ...
  return (
    ...
    >
      <div className="relative aspect-[16/7] bg-muted overflow-hidden">
        ...
        {p.category?.name && (
          <span className="absolute top-1 right-1 rounded bg-panel px-1 py-0.5 text-[9px] font-bold text-panel-foreground max-w-[70%] truncate">
            {p.category.name}
          </span>
        )}
        {hasDiscount && (
          <span className="absolute bottom-1 left-1 rounded bg-discount px-1 py-0.5 text-[9px] font-bold text-discount-foreground">
            -{p.discount_percent}%
          </span>
        )}
      </div>

      <div className="px-2 py-1.5 flex flex-col gap-0 min-w-0 border-t border-border">
        <h3 className="text-[10px] font-bold line-clamp-1 min-w-0">{p.name}</h3>

        <div className="flex items-end justify-between gap-1.5 min-w-0">
          <div className="flex items-baseline gap-1 min-w-0">
            <span className="text-[10px] font-black text-card-foreground truncate">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            >
              <Heart className="size-3" fill={fav ? "currentColor" : "none"} />
            </button>
          </div>
        </div>
      </div>
