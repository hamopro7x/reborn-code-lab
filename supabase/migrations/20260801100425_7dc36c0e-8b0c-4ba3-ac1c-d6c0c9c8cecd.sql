ALTER TABLE public.agent_devices ADD COLUMN IF NOT EXISTS app_version text;

CREATE OR REPLACE FUNCTION public.agent_heartbeat(p_device_id text, p_secret text, p_version text DEFAULT NULL)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.agent_register(p_device_id text, p_secret text, p_employee_name text, p_device_label text, p_os text, p_version text DEFAULT NULL)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
