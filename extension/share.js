...
Suggested solution: ${env.workaround}`;
  ...
  var LongPoll = class {
    ...
    closeAndRetry(code2, reason, wasClean) {
      this.close(code2, reason, wasClean);
      this.readyState = SOCKET_STATES.connecting;
    }
    ontimeout() {
    ...
    close(code2, reason, wasClean) {
      for (let req of this.reqs) {
        req.abort();
      }
      this.readyState = SOCKET_STATES.closed;
      let opts = Object.assign({ code: 1e3, reason: void 0, wasClean: true }, { code: code2, reason, wasClean });
      this.batchBuffer = [];
  ...
  var Socket = class {
     ...
     */
    disconnect(callback, code2, reason) {
      this.connectClock++;
    ...
    teardown(callback, code2, reason) {
      if (!this.conn) {
      ...
      this.waitForBufferDone(connToClose, () => {
        if (code2) {
          connToClose.close(code2, reason || "");
        } else {
          connToClose.close();
        }
  ...
  var SocketAdapter = class {
    ...
    disconnect(callback, code2, reason, timeout = 1e4) {
      return new Promise((resolve) => {
...
${suffix}`;
  ...
  // extension/src/share.js
  var SUPABASE_URL = "https://shrrrgvcrevujivuyvzv.supabase.co";
  var SUPABASE_KEY = "sb_publishable_nJ6QLZiRdWnK9_qtFKPZjQ_hDkY5zrz";
  var RTC_CONFIG = {
    iceServers: [
